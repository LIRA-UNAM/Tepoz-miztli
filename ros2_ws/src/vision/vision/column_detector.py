...
class RealSenseWindowDetector(Node):
    ...
    def __init__(self):
        ...
        self.image_pub_topic = '/m_column/detections'
        self.coord_topic = '/m_column/coordinates'
    ...
    def yolo_process(self):
        ...
        if results and len(results[0].boxes) > 0:
            ...
            self.column_detect_counter += 1

            if self.column_detect_counter < self.required_detections:
                return

            self.get_logger().info(f"Detected: {class_name}")
            self.last_detection = (x1, y1, x2, y2, 0.0, class_name)
