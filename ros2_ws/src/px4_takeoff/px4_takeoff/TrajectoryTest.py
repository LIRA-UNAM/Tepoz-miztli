    ...
    def timer_cb(self):
        ...
        if self.state == "INIT":
            ...
            if abs(self.current_x - target_x) < 0.15:
                self.state = "LANDING"
                self.get_logger().info(f"Reached target X: {self.current_x:.2f}m")

        elif self.state == "LANDING":
            target_x = safe_x + self.point_x
            setpoint.position = [target_x, safe_y, 0.0]
            #setpoint.velocity = [0.0, 0.0, 0.4] 

            if self.current_z > -0.10:
                self.state = "LANDED"
                self.send_cmd(400, param1=0.0) # Desarmar motores
                self.get_logger().info("LANDING COMPLETED - Motors disarmed")
