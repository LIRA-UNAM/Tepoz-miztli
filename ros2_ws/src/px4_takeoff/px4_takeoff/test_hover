...
class PX4FlowPrecision(Node):
    def __init__(self):
        ...
        # Nota: PX4 usa sistema NED (Z negativo es hacia arriba)
        self.target_z = -1.2 
    ...
    def timer_cb(self):
        ...
        offboard.velocity = True # Activado para evitar rechazos en takeoff/land en v1.17
        ...
        # Capturamos la posición real justo antes de despegar
        ...
        # Cero en lugar de NaN para cumplir con los chequeos de seguridad de v1.17
        ...
        if self.state == "INIT":
            ...
            if self.counter > 30:
                ...
                self.get_logger().info("ARMED - Ascendiendo")
                self.state = "TAKEOFF"
            ...
            setpoint.position = [safe_x, safe_y, 0.0] # 0.0 asume que el origen está en el piso
            setpoint.velocity = [0.0, 0.0, 0.4] # Velocidad de descenso controlada (Z positivo es bajar)
...
# Se agrega args=None para que ROS 2 reconozca correctamente el punto de entrada
def main(args=None):
