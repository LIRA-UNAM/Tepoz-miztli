...
class Mision1_Full(Node):
    def __init__(self):
        ...
        self.gates_green = self.create_subscription(
            PoseArray,
            'm1/green/coordinates',
            self.gate_cb, 1
        )
        ...
        self.get_logger().info(
            f"Paquete de Misiones circuito completo de Gate Green Iniciado"
        )
    ...
    def timer_cb(self):
        ...
        if self.state == 'INIT':
            ...
            if elapsed >= HOLD_DURATION:
                self.get_logger().info('SEARCH_GATE — buscando gate verde')
